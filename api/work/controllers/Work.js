...
module.exports = {
  ...
  previewOne: async ctx => {
    ...
    // var template = fs.readFileSync('./public/engine.client.ejs', 'utf-8')
    // var html = ejs.render(template, { work })

    // fs.writeFileSync('./public/index.html', html, 'utf8')

    const res = ctx.render('engine', { work })
    return res
  },
  ...
  queryWorksWithForms: async ctx => {
    ...
    workRecords = workRecords
      ...
      .map(work => ({
        ...work,
        form_count:
          groupedFormRecords[work.id] && groupedFormRecords[work.id].length
      }))
  ...
  useTemplate: async ctx => {
    ...
    return strapi.services.work.update(
      { id: work.id },
      { pages: templateWork.pages, is_template: false }
    )
  },
  uploadPSD: async ctx => {
    const pageJSON = await strapi.services.work.parsePSD(
